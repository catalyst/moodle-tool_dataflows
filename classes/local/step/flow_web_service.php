...
class flow_web_service extends flow_logic_step {
    ...
    public function form_add_custom_inputs(\MoodleQuickForm &$mform) {
        ...
        $outputexample['somekey'] = \html_writer::nonempty_tag('span', 'steps.' . ($this->stepdef->alias ?? 'alias') . '.somekey', $class);
        $outputexample['id'] = \html_writer::nonempty_tag('span', 'steps.' . ($this->stepdef->alias ?? 'alias') . '.id', $class);
        $outputexample['username'] = \html_writer::nonempty_tag('span', 'steps.' . ($this->stepdef->alias ?? 'alias') . '.username', $class);
        ...
        if (!$isdryrun) {
            ...
            if ($response['error']) {
                ...
                if ($failure === 'record') {
                    ...
                    file_put_contents($destination, $response['exception'] ?? $response['exception']->message);
                    return false;
                }
            }
